...
Description: Analizza l'ultimo immobile creato (il Test Listing) per verificare i campi.
Version: 1.1
*/
...
function wasi_analyze_last_post() {
    ...
    if ( empty($posts) ) return "Nessun immobile trovato. Fai prima un import manuale!";

    $immobile = $posts[0];
    $id = $immobile->ID;
    $meta = get_post_meta($id);

    $output = "<h3>Analisi Immobile: " . $immobile->post_title . " (ID: $id)</h3>";
    ...
    foreach ($meta as $key => $values) {
        ...
        // Evidenziamo i campi critici in verde
        $style = "";
        if($key == '_price' || $key == 'wasi_id' || $key == '_address') $style = "style='background:#eaffea; font-weight:bold;'";

        $output .= "<tr $style><td><strong>$key</strong></td><td>$val</td></tr>";
    }
    $output .= "</table>";
